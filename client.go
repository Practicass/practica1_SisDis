...
func sendRequest(endpoint string, id int, interval com.TPInterval, addChan chan com.TimeRequest, delChan chan com.TimeReply, finChan chan int) {
...
func handleRequests(addChan chan com.TimeRequest, delChan chan com.TimeReply, finChan chan int) {
...
func main() {
	...
	finChan := make(chan int, 60)
	go handleRequests(addChan, delChan, finChan)

	for i := 0; i < numIt; i++ {
		for t := 1; t <= requestTmp; t++ {

			sendRequest(endpoint, i*requestTmp+t, interval, addChan, delChan, finChan)
		}
